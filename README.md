...
- **[OB-1](https://github.com/Overbrilliant/ob-1)** `⭐ 16`
- **[Sabi](https://github.com/vizuh/sabi)** `⭐ 15` — Adaptive per-round inference scheduling for coding agents: sits between the harness and its model providers and routes each inference round — not each session — against trajectory state. Round position, tool calls and their results, failure evidence and context pressure classify the round, and a deterministic policy maps class to tier (`failure` → strong, `exploration` → cheap, `first-turn` → mid). An optional semantic judge runs only on rounds the policy cannot classify, and is fail-open; the harness loop is never forked or patched. Exposes an OpenAI-compatible local proxy (`npm start` → `http://127.0.0.1:8787/v1`); from OpenCode, `npm run connect:opencode` writes only a `provider.sabi` entry and the adaptive alias is `sabi/sabi-code`, with `sabi/sabi-cheap/-mid/-strong` as fixed baselines. Decisions, tokens and cost are inspectable via `npm run report`. MIT.
 — Terminal coding agent that runs with no account, API key, or card against a free-model catalog. Persistent project memory builds a fact and relationship graph from real work and surfaces it with `/memory`. npm `ob1`. Apache-2.0.
